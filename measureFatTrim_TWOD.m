...
for lambda = [1,2^-1,2^-2,2^-3,2^-4,2^-5]
    ...
    %norm = [];
    for k = 1:n
        SIGMAk = kthcoverH_twod(k,lambda);
        SIGMAk_next = kthcoverH_twod(k+1,lambda);
    ...
    plot(K,measures,'.-','markersize',22,'linewidth',1.5)
    %semilogy(K,measures,'.-','markersize',22)
    %semilogy(K,norm,'.-','markersize',22)
    hold on
end
